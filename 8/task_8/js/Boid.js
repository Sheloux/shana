...
// import Easing from "./EasingTools.js";

export default class Boid {
    constructor(x, y, s, ctx, target) {
        this.position = { x, y }
        this.origin = { x, y }
        this.size = s
        this.color = "black"
        ...
        // this.imageElement.src = 'assets/Blacky.png';
        // this.Isize = { x: this.imageElement.width, y: this.imageElement.height };

        // this.imageElement.onload = function () {
        //     document.body.appendChild(this.imageElement);
        //     console.log("Little blackies have been downloaded !");
        // };
    ...
    display(target) {
        ...
        // this.ctx.drawImage(this.imageElement, this.position.x + this.Isize.x / 2, this.position.y - this.Isize.y, this.Isize.x, this.Isize.y);

        this.creatures(this.position.x, this.position.y, 40, target);
        // this.circle(this.position.x, this.position.y, this.size / 1.5);
    ...
    separate(boids, desiredSeparation = 35) {
    ...
    seek(target) {
        let desired = { x: target.x * this.ctx.canvas.width - this.position.x, y: target.y * this.ctx.canvas.height - this.position.y }
    ...
    creatures(centerX, centerY, target) {
        const lineCount = 60;
        for (let i = 0; i < lineCount; i += 1) {
            const angle = (i / lineCount) * 2 * Math.PI;
            const startX = centerX + this.size * Math.cos(angle);
            const startY = centerY + this.size * Math.sin(angle);

            const endX = centerX + this.size * Math.cos(angle + Math.PI);
            const endY = centerY + this.size * Math.sin(angle + Math.PI);
            ...
            if (target.x != null || target.y != null) {
                ...
                this.circle(centerX + this.size * 0.4, centerY - this.size * 0.2, this.size * 0.4);
                this.circle(centerX - this.size * 0.4, centerY - this.size * 0.2, this.size * 0.4);

                this.ctx.fillStyle = 'black';

                this.circle(centerX + this.size * 0.4, centerY - this.size * 0.2, this.size * 0.3);
                this.circle(centerX - this.size * 0.4, centerY - this.size * 0.2, this.size * 0.3);

            } else {
                this.ctx.fillStyle = 'white';
                this.circle(centerX + this.size * 0.4, centerY - this.size * 0.2, this.size * 0.2);
                this.circle(centerX - this.size * 0.4, centerY - this.size * 0.2, this.size * 0.2);

                this.ctx.fillStyle = 'black';

                this.circle(centerX + this.size * 0.4, centerY - this.size * 0.2, this.size * 0.1);
                this.circle(centerX - this.size * 0.4, centerY - this.size * 0.2, this.size * 0.1);
            }
        }
    }
